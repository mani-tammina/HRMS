...
export interface ProfileLeaveCard {
  title: string;
  code: string;
...
export class LeavesTabComponent implements OnInit, OnChanges, OnDestroy {
  ...
  constructor(private employeeLeavesService: EmployeeLeavesService) {}
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes['currentEmployee'] && !changes['currentEmployee'].firstChange) {
      this.loadLeaveBalance();
    }
  }
  ...
  loadLeaveBalance() {
    ...
    this.employeeLeavesService.getLeaveBalance(this.currentYear, Number(this.currentEmployee.id))
      ...
      .subscribe({
        ...
        error: (err: any) => {
          this.isLoading = false;
          console.error('Failed to load employee leave balance:', err);
          this.errorMessage = 'Failed to load leave balances for this employee.';
        }
      });
  }
